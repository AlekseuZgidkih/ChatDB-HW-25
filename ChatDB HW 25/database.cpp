...
bool DatabaseManager::connectToDatabase() {
    ...
    // Âûäåëÿåì ðåñóðñû äëÿ îêðóæåíèÿ ODBC
    ...
    // Âûäåëÿåì ðåñóðñû äëÿ ïîäêëþ÷åíèÿ ê áàçå äàííûõ
    ...
    // Âîçâðàùàåì ðåçóëüòàò óñïåøíîñòè ïîäêëþ÷åíèÿ
...
void DatabaseManager::disconnectFromDatabase() {
    ...
    // Îñâîáîæäàåì îáðàáîò÷èê âûðàæåíèÿ, åñëè îí áûë âûäåëåí
    if (hstmt) {
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
    }

    // Îòêëþ÷àåìñÿ îò áàçû äàííûõ è îñâîáîæäàåì îáðàáîò÷èê ñîåäèíåíèÿ
    if (hdbc) {
        SQLDisconnect(hdbc);
    ...
    // Îñâîáîæäàåì îáðàáîò÷èê îêðóæåíèÿ
    if (henv) {
        SQLFreeHandle(SQL_HANDLE_ENV, henv);
    }
}
...
bool DatabaseManager::insertDataIntoTable() {
    if (connectToDatabase()) {
        ...
        // Äîáàâëåíèå äàííûõ â òàáëèöó messages
